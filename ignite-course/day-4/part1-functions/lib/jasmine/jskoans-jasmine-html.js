...
JsKoansReporter.prototype.reportRunnerStarting = function(runner) {
  this.outerDiv = this.createDom('div', { className: 'jasmine_reporter show-passed' },
      this.createDom('img', {src:"public/learn_to_code_manchester_original_second.png", width:"400px"}),
...
JsKoansReporter.prototype.reportSpecResults = function(spec) {
  ...
  if ( !results.passed() ) {
    ...
    description = "It \"" + spec.description + "\""
  } else {
  ...
  for (var i = 0; i < resultItems.length; i++) {
    ...
    if(result.trace.stack){
      var stack = result.trace.stack.split('at')[4];
      var fileAndLineArray = stack.split('/');
      var lastElement = fileAndLineArray[fileAndLineArray.length -1];
      var trimmed = lastElement.substring(0, lastElement.length - 9);
      var lineMessage = trimmed.split(':').join(' line ');
    }
    if (result.type == 'expect' && result.passed && !result.passed()) {
      ...
      messagesDiv.appendChild(
          this.createDom('div', { className: 'description' }, "Please fix " + lineMessage)
          );
